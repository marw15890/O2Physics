    ...
    for (auto& lcCand : lcCands) {
      ...
      for (auto& Prong3Cand : Prong3BaseCands) {
        ...
        if (Prong3Cand.sign() > 0) {
          continue;
        }    /// prong3cand has a sign function???
        if (Prong3Cand.globalIndex() == index0Lc || Prong3Cand.globalIndex() == index1Lc || Prong3Cand.globalIndex() == index2Lc) {
          continue;
        }
        hPtPion1->Fill(Prong3Cand.index0_as<aod::BigTracks>().pt() < ptPionMin);
        array<float, 3> pvecPion;
        auto trackParVarPi = getTrackParCov(trackPion);

        hPtkaon->Fill(Prong3Cand.index1_as<aod::BigTracks>().pt());
        array<float, 3> pvecPion;
        auto trackParVarPi = getTrackParCov(trackPion);

        hPtPion->Fill(Prong3Cand.index2_as<aod::BigTracks>().pt());
        array<float, 3> pvecPion;
        auto trackParVarPi = getTrackParCov(trackPion);
